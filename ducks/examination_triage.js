...
const EXAM_TRIAGE_PATIENT_RECORD = 'EXAM_TRIAGE_PATIENT_RECORD'
...
export function examinationTriages(state = initState, action = {}) {
  switch (action.type) {
    ...
    case EXAM_TRIAGE_PATIENT_RECORD:
...
export const ExaminationTriagePatientRecordList = ({ patient_id, clinic_triage_patient_id, offset, limit }) => async dispatch => {
  try {
    console.log('limit====', { patient_id, clinic_triage_patient_id, offset, limit })
    const data = await request('/examinationTriage/ExaminationTriagePatientRecordList', { patient_id, clinic_triage_patient_id, offset, limit })
    console.log('data=====', data)
    const docs = data.data || []
    const page_info = data.page_info || {}
    dispatch({
      type: EXAM_TRIAGE_PATIENT_RECORD,
