...
class CMedicinePrescriptionTemplateScreen extends Component {
  constructor(props) {
    super(props)
    this.state = {
      drugClassification: [],
  ...
  getDataList({ offset = 0, limit = 10 }) {
    const {clinic_id, PrescriptionChinesePatientModelList} = this.props
    ...
    PrescriptionChinesePatientModelList(requestData, true)
  }
  // 状态筛选
  getStatusOptions() {
    return [
  ...
  // 加载表格
  renderTable() {
    const { prescriptionChinesePatientModels, pageInfo } = this.props
    console.log('prescriptionChinesePatientModels=====', prescriptionChinesePatientModels)
    return (
...
const mapStateToProps = state => {
  return {
    ...
    pageInfo: state.prescriptionChinesePatientModels.page_info,
