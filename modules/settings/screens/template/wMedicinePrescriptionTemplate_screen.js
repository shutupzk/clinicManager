...
import AddPrescriptionWesternPatientModelscreen from './components/addPrescriptionWesternPatientModelscreen'
import moment from 'moment'

class WMedicinePrescriptionTemplateScreen extends Component {
  constructor(props) {
    super(props)
    this.state = {
      drugClassification: [],
  ...
  showView() {
    ...
    let map = {
      // 1: <AddDrugScreen />,
      2: <AddPrescriptionWesternPatientModelscreen
        drugType={1}
        showWay={showWay}
  ...
  getDataList({ offset = 0, limit = 10 }) {
    const {clinic_id, PrescriptionWesternPatientModelList} = this.props
    ...
    PrescriptionWesternPatientModelList(requestData, true)
  }
  // 状态筛选
  getStatusOptions() {
    return [
  ...
  // 加载表格
  renderTable() {
    const { prescriptionWesternPatientModels, pageInfo } = this.props
    console.log('prescriptionWesternPatientModels=====', prescriptionWesternPatientModels)
    return (
            ...
            {prescriptionWesternPatientModels.map((item, index) => {
              ...
              return (
                <tr key={index}>
                  ...
                  <td style={{flex: 2}} title={items}>{items}</td>
                  ...
                  <td style={{flex: 2}} className={'operTd'}>
                    <div>
                      ...
                      <div onClick={() => {
                        this.PrescriptionWesternPatientModelDelete(item.prescription_patient_model_id)
                      }}>删除</div>
                    </div>
                  </td>
                </tr>
  ...
  // 删除
  PrescriptionWesternPatientModelDelete(prescription_patient_model_id) {
    const {PrescriptionWesternPatientModelDelete, pageInfo, prescriptionWesternPatientModels} = this.props
    this.refs.myAlert.confirm('提示', '确认删除这条记录？', 'Warning', async () => {
      let error = await PrescriptionWesternPatientModelDelete({prescription_patient_model_id})
      if (error) {
        return this.refs.myAlert.alert('删除失败', error)
      } else {
        this.refs.myAlert.alert('删除成功')
        if (prescriptionWesternPatientModels.length > 1) {
...
const mapStateToProps = state => {
  return {
    ...
    pageInfo: state.prescriptionWesternPatientModels.page_info,
...
export default connect(mapStateToProps, {
  ...
  queryDoctorList,
  PrescriptionWesternPatientModelDelete
})(WMedicinePrescriptionTemplateScreen)
