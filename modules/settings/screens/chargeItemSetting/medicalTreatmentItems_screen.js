...
class MedicalTreatmentItemsScreen extends Component {
  constructor(props) {
    super(props)
    this.state = {
      drugClassification: [],
  ...
  getDataList({ offset = 0, limit = 10 }) {
    const {clinic_id, queryDiagnosisTreatmentList} = this.props
    ...
    queryDiagnosisTreatmentList(requestData, true)
  }
  // 状态筛选
  getStatusOptions() {
    return [
  ...
  // 加载表格
  renderTable() {
    const { diagnosisTreatments, pageInfo } = this.props
    console.log('diagnosisTreatments=====', diagnosisTreatments)
    return (
      <div className={'tableContent'}>
        <table>
          <thead>
            <tr>
              <td style={{flex: 2}}>诊疗项目名称</td>
              <td>诊疗费金额</td>
              <td>成本价</td>
          ...
          <tbody>
            {diagnosisTreatments.map((item, index) => {
              return (
                <tr key={index}>
