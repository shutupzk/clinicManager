...
class WMPrescriptionScreen extends Component {
  ...
  async componentWillMount() {
    ...
    let drugClass = await this.props.queryDrugClassList({ clinic_id })
    let drug_class_id = -1
    if (drugClass) {
      drug_class_id = drugClass.id
      this.setState({ drug_class_id })
    }
    this.getDrugsList({ offset: 0, limit: 10, drug_class_id })
  }
  showView() {
    let { pageType, drug_class_id } = this.state
    const { clinic_id } = this.props
    let map = {
      // 1: <AddDrugScreen />,
      2: (
        <AddDrugScreen
          ...
          back2List={() => {
            ...
            this.props.queryDrugClassList({ clinic_id })
          }}
        />
      )
    }
  ...
  renderLeftTree() {
    ...
    const drugClassification = this.getDrugClassification() || []
    return (
          ...
          {drugClassification.map((item, index) => {
            return (
              <li
                key={index}
                onClick={() => {
                  this.setState({ drug_class_id: item.id }, function() {
                    this.getDrugsList({})
                  })
                }}
              >
                <div>
                  <i />
                  <span style={{color: drug_class_id === item.id ? '#2ACDC8' : 'rgba(102, 102, 102, 1)'}}>{item.name}</span>
                  <a style={{color: drug_class_id === item.id ? '#2ACDC8' : 'rgba(102, 102, 102, 1)'}}>{item.count}</a>
                </div>
              </li>
            )
          })}
...
export default connect(mapStateToProps, { ClinicDrugList, queryDrugClassList })(WMPrescriptionScreen)
