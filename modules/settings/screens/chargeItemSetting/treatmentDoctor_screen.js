...
class TreatmentDoctorScreen extends Component {
  ...
  showView() {
    let { pageType, showWay, clinic_treatment_id } = this.state
    let map = {
      // 1: <AddDrugScreen />,
      2: <AddTreatmentScreen
        drugType={1}
        showWay={showWay}
  ...
  getDataList({ offset = 0, limit = 10 }) {
    const {clinic_id, queryTreatmentList} = this.props
    ...
    queryTreatmentList(requestData, true)
  }
  // 状态筛选
  getStatusOptions() {
    return [
                  ...
                  <td style={{flex: 2}} className={'operTd'}>
                    <div>
                      ...
                      <div onClick={() => {
                        ...
                        this.TreatmentOnOff(item.clinic_treatment_id, status)
  ...
  async TreatmentOnOff(clinic_treatment_id, status) {
    const {clinic_id, TreatmentOnOff, pageInfo} = this.props
    const requestData = {
      clinic_treatment_id,
      clinic_id,
      status
    }
    let error = await TreatmentOnOff(requestData)
    if (error) {
