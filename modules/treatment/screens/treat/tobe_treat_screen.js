...
class TobeTreatScreen extends Component {
  ...
  // 显示待收费
  showTobeCharged() {
    const { waiting_data, pageInfo, TreatmentTriageList } = this.props
    return (
      <div>
              ...
              return (
                <li key={index}>
                  ...
                  <div className={'itemBottom'}>
                    <span
                      onClick={async () => {
                        let treatments = await TreatmentTriageList({ clinic_triage_patient_id: patient.clinic_triage_patient_id, order_status: '10' })
                        console.log('treatments ======', treatments)
                        this.setState({ showMask: true, selItem: patient, treatments: treatments || [] })
                      }}
                    >
                      待治疗
                    </span>
  ...
  async determineTreat() {
    const { selItem, items } = this.state
    const { TreatmentTriageUpdate } = this.props
    let array = []
    for (let treatment_patient_id in items) {
      array.push({ treatment_patient_id: treatment_patient_id })
    }
    const { clinic_triage_patient_id } = selItem
    const order_status = '20'
    let error = await TreatmentTriageUpdate({ clinic_triage_patient_id, order_status, items: JSON.stringify(array) })
    if (error) {
  ...
  // 确认检查
  renderDetermine() {
    const { selItem, treatments, items } = this.state
                        ...
                        <td>
                          <input
                            ...
                            onChange={e => {
                              ...
                              if (!checked) {
                                delete items[item.treatment_patient_id]
                              } else {
                                items[item.treatment_patient_id] = item.treatment_patient_id
                              }
                              this.setState({ items })
                            }}
                          />
                        </td>
                        <td style={{ flex: 9 }}>{item.clinic_treatment_name}</td>
                      </tr>
                    )
                  })}
                </tbody>
    ...
    return (
      <div>
        <div className={'childTopBar'}>
          ...
          <span onClick={() => Router.push('/treatment/treat/checked')}>已治疗</span>
        </div>
