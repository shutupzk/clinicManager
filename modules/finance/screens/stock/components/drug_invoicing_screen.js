...
// 其他收费
class DruginvoicingScreen extends Component {
  constructor(props) {
    super(props)
    this.state = {
      start_date: moment()
  ...
  queryContentData({ offset = 0, limit = 10 }) {
    const { DrugInvoicingStatistics, clinic_id } = this.props
    const {
      start_date,
      end_date,
      supplier_name,
    ...
    DrugInvoicingStatistics(reqData)
  }
  // 获取供应商筛选
  getSupplierOptions() {
...
const mapStateToProps = state => {
  return {
    invoice_data: state.invoicingStatistics.div_data,
    pageInfo: state.invoicingStatistics.div_page_info,
