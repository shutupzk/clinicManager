...
import { TreatmentStatistics } from '../../../../../ducks'
...
class Treatmentscreen extends Component {
  ...
  queryContentData({ offset = 0, limit = 10 }) {
    const { TreatmentStatistics, clinic_id } = this.props
    const { start_date, end_date } = this.state
    TreatmentStatistics({ start_date, end_date, offset, limit, clinic_id })
  }
            ...
            {data.map((item, iKey) => {
              return (
                <li key={iKey}>
                  ...
                  <div>{item.clinic_treatment_name}</div>
                  <div>次</div>
                  <div>{item.times}</div>
...
const mapStateToProps = state => {
  return {
    ...
    data: state.medReports.treat_data,
    page: state.medReports.treat_page
  }
}
