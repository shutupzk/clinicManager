...
import { ExaminationStatistics } from '../../../../../ducks'
...
class Examinationscreen extends Component {
  ...
  queryContentData({ offset = 0, limit = 10 }) {
    const { ExaminationStatistics, clinic_id } = this.props
    const { start_date, end_date } = this.state
    ExaminationStatistics({ start_date, end_date, offset, limit, clinic_id })
  }
            ...
            {data.map((item, iKey) => {
              return (
                <li key={iKey}>
                  ...
                  <div>{item.clinic_examination_name}</div>
                  <div>次</div>
                  <div>{item.times}</div>
...
const mapStateToProps = state => {
  return {
    ...
    data: state.medReports.exam_data,
    page: state.medReports.exam_page
  }
}
