...
import AddMaterialInstockScreen from './components/addMaterialInstockScreen'
import moment from 'moment'
...
class StorageMgtScreen extends Component {
  ...
  getDataList({ offset = 0, limit = 10 }) {
    const {clinic_id, queryMaterialInstockRecord} = this.props
    ...
    queryMaterialInstockRecord(requestData)
  }
  async MaterialInstockRecordDelete(material_instock_record_id) {
    const { MaterialInstockRecordDelete, pageInfo, materailStocks } = this.props
    this.refs.myAlert.confirm('提示', '确认删除这条记录？', 'Warning', async () => {
      let error = await MaterialInstockRecordDelete({material_instock_record_id})
      if (error) {
        return this.refs.myAlert.alert('删除失败', error)
      } else {
        this.refs.myAlert.alert('删除成功')
        if (materailStocks.length > 1) {
  ...
  renderTable() {
    const { materailStocks, pageInfo } = this.props
    console.log('materailStocks=====', materailStocks)
    return (
                      ...
                      <td style={{flex: 2}} className={'operTd'}>
                        {item.verify_status === '01' ? <div>
                          <div onClick={() => {
                            this.setState({
                              showType: 2,
                              material_instock_record_id: item.material_instock_record_id,
                              showWay: 4
                            })
                          }}>修改</div>
                          ...
                          <div className={'divideLine'}>|</div>
                          <div onClick={() => {
                            this.MaterialInstockRecordDelete(item.material_instock_record_id)
                          }}>删除</div>
                        </div> : <div>
                          <div onClick={() => {
                            this.setState({
  ...
  showView() {
    let { showType, material_instock_record_id, showWay } = this.state
    let map = {
      // 1: <AddDrugScreen />,
      2: <AddMaterialInstockScreen showWay={showWay} material_instock_record_id={material_instock_record_id} drugType={1} backToList={() => {
    ...
    return (
      <div className={'filterBox'}>
        ...
        <div className={'boxRight'}>
          <button
            onClick={() => {
              this.setState({showType: 2, showWay: 1, material_instock_record_id: ''})
            }}
          >
            新增入库
          </button>
...
const mapStateToProps = state => {
  return {
    ...
    materailStocks: state.materailStocks.data,
    pageInfo: state.materailStocks.page_info
  }
}
