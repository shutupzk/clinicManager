...
export class SelectFilterCard extends Component {
  render () {
    ...
    return (
      <div className='left select flex tb-flex' style={{ border: '1px solid #E6E6E6', minWidth: 100, background: 'none', borderRadius: 4, width: 'auto' }}>
        <select
          onChange={e => {
        ...
        >
          ...
          {this.props.data &&
            this.props.data.map((item, iKey) => {
              return (
                <option value={item[config.valueKey]} key={iKey}>
                  {item[config.titleKey]}
                </option>
              )
            })}
        </select>
        <article className='select-icon'>
          <i />
          <i />
        </article>
      </div>
    ...
    return (
      <section className='left' style={{ border: '1px solid #E6E6E6', borderRadius: 4, margin: '0 .15rem' }}>
        <input
          ...
          onKeyUp={e => {
            if (e.keyCode === 13) {
              this.props.clickfilter(this.refs.keywordRef && this.refs.keywordRef.value)
            }
          }}
        />
        <button className='left btnBGGray btnBGLitt' style={{ height: '.34rem', lineHeight: '.34rem' }} onClick={() => this.props.clickfilter(this.refs.keywordRef && this.refs.keywordRef.value)}>
          搜索
        </button>
        <div className='clearfix' />
      </section>
...
export class LinkQuestion extends Component {
  render () {
    return (
      <a style={{ color: theme.fontcolor, fontSize: theme.nfontsize, display: 'none' }} className='right'>
        问题和帮助QA
      </a>
    )
  }
